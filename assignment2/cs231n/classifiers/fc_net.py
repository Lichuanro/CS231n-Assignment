...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        h_out, h_cache = affine_forward(X, self.params['W1'], self.params['b1'])
        relu_out, relu_cache = relu_forward(h_out)

        scores, final_cache = affine_forward(np.asarray(relu_out), self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dx2, dw2, db2 = affine_backward(dscores, final_cache)
        grads['W2'] = dw2 + self.reg * self.params['W2']
        grads['b2'] = db2

        dx2_relu = relu_backward(dx2, relu_cache)
        dx1, dw1, db1 = affine_backward(dx2_relu, h_cache)
